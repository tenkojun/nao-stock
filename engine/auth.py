...
성격: 개인 PC에서 도는 앱이므로 '보안 방벽'이라기보다 **누가 쓰는지 구분**하는 장치.
  · 아버지 계정 — 비밀번호 없이 클릭 한 번으로 입장(60대 사용성 우선)
...
세션: 토큰을 발급해 클라이언트가 보관. 만료 30일(아버지가 매번 로그인하지 않도록).
"""
...
def ensure_users(admin_pw=None):
    ...
    if d:
        return d
    d = {"users": {
        "father": {"name": "아버지", "role": "user", "no_password": True,
                   "salt": "", "hash": "", "created": time.strftime("%Y-%m-%d")},
        "admin": {"name": "관리자(아들)", "role": "admin", "no_password": admin_pw is None,
                  "salt": "", "hash": "", "created": time.strftime("%Y-%m-%d")},
    }}
    if admin_pw:
        s, h = _hash(admin_pw)
